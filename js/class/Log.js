...
Log.prototype.setAction = function (action) {
    this.action = action;
}
...
Log.prototype.createContent = function () { //<b>넣기
    ...
    if (this.action == "update_WB") {
        this.content =
            `<b>${this.category_title}</b>의 <b>${this.prev_work_box_title}</b>가 <b>${this.work_box_title}</b>(으)로 <b>변경</b>하였습니다.`
    }
    if (this.action == "update_CT") {
        this.content =
            `<b>${this.prev_category_title}</b>가 <b>${this.category_title}</b>(으)로 <b>변경</b>하였습니다.`
    }
    if (this.action == "remove_WB") {
        this.content =
